...
class ContactStatus(str):
    ...
    def normalize(cls, value: str) -> str:
        ...
        if value_upper in cls.get_valid_values():
            return value_upper
        mapping = {
            'STRING': cls.NEW, 'NEW': cls.NEW, 'MET': cls.MET, 'INTERESTED': cls.INTERESTED,
...
class CommunicationType(str):
    ...
    def normalize(cls, value: str) -> str:
        ...
        if value_upper in cls.get_valid_values():
            return value_upper
        mapping = {
            'CALL': cls.CALL, 'MEETING': cls.MEETING, 'EMAIL': cls.EMAIL, 'MESSAGE': cls.MESSAGE,
...
class CommunicationStatus(str):
    ...
    def normalize(cls, value: str) -> str:
        ...
        if value_lower in cls.get_valid_values():
            return value_lower
        mapping = {
            'planned': cls.PLANNED, 'completed': cls.COMPLETED, 'cancelled': cls.CANCELLED, 'missed': cls.MISSED,
    ...
    for group_config in GROUPS_CONFIG:
        ...
        if not profile:
            ...
            stats = {
                ...
                "budget": {
                    "total": profile.budget_places or 0, "filled": profile.budget_filled or 0,
                ...
                "paid": {
                    "total": profile.paid_places or 0, "filled": profile.paid_filled or 0,
                    "free": profile.paid_free or 0, "applicants_with_consent": profile.paid_applicants_with_consent or 0
                },
                "target": {
                    "total": profile.target_places or 0, "filled": profile.target_filled or 0,
...
@router.get("/communications/stats", response_model=dict)
