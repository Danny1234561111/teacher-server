...
async def mobile_get_students(
        ...
        department_id: Optional[int] = None,
        speciality_id: Optional[int] = None,
        study_form: Optional[str] = Query(None, description="Форма обучения"),
        study_basis: Optional[str] = Query(None, description="Основа обучения"),
...
async def mobile_get_student_applications(
    ...
    if current_user.role != UserRole.ADMIN and student.kurator_id != current_user.id:
...
async def mobile_get_student_competitive_info(
        student_id: int,
        current_user: User = Depends(get_current_user_mobile),
        db: Session = Depends(get_db)
):
    ...
    if not main_application:
        return CompetitiveInfoResponse(
            ...
            speciality_name=None, profile_name=None, study_form=None, study_basis=None,
            budget_places_total=None, budget_places_filled=None, budget_places_free=None, competition=None
        )

    # ... остальная логика конкурсной информации ...
    return CompetitiveInfoResponse(
        position=1, total_students=0, total_enrolled=0, total_submitted=0,
        average_score=0, min_score=0, max_score=0, passing_score=None,
        student_score=None, department_name=None, speciality_name=None,
        profile_name=None, study_form=None, study_basis=None,
        budget_places_total=None, budget_places_filled=None,
...
async def web_get_student(
    ...
    current_user = await get_current_user_web(request, db)
    student = student_service.get_student_by_id(
        student_id=student_id, user_id=current_user.id, db=db
    )
    if not student:
        raise HTTPException(status_code=404, detail="Абитуриент не найден или доступ запрещен")
    return student
...
async def web_get_student_applications(
    ...
    for app in applications:
        ...
        result.append(StudentApplicationResponse(
            id=app.id, student_id=app.student_id,
            department_id=app.department_id,
            department_name=department.name if department else None,
            speciality_id=app.speciality_id,
            speciality_name=speciality.name if speciality else None,
            profile_id=app.profile_id,
            profile_name=profile.name if profile else None,
            position=app.position, priority=app.priority,
            total_score=app.total_score,
            application_status=app.application_status.value if app.application_status else None,
            consent_status=app.consent_status if app.consent_status is not None else False,
            participation=app.participation if app.participation is not None else True,
